(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('jhipsterTestApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-jhipsterTestApp-error');
                var entityKey = httpResponse.headers('X-jhipsterTestApp-params');
                if (errorHeader) {
